...
impl<T: ProcessScene> sharedparley::GlyphRenderer for SceneBuilder<'_, T> {
    ...
    fn fill_rectangle(&mut self, mut physical_rect: sharedparley::PhysicalRect, color: Color) {
        ...
        let global_offset = self.current_state.offset.to_vector().cast() * self.scale_factor;
        physical_rect.origin += global_offset;
        let clip = self.current_state.clip.translate(self.current_state.offset.to_vector()).cast()
            * self.scale_factor;
        let Some(physical_rect) = physical_rect.intersection(&clip) else { return };
    ...
    fn draw_glyph_run(
        ...
        // TextInput can be much taller than the i16 framebuffer coordinates.
        // Combine the scroll and glyph positions before narrowing, and discard
        // offscreen glyphs while their coordinates are still floating point.
        let global_offset = self.current_state.offset.to_vector().cast::<f32>() * self.scale_factor;
        let clip = self.current_state.clip.translate(self.current_state.offset.to_vector()).cast::<f32>()
            * self.scale_factor;
        ...
        for positioned_glyph in glyphs_it {
            ...
            let glyph_offset: euclid::Vector2D<f32, PhysicalPx> = euclid::Vector2D::from_lengths(
                euclid::Length::new(positioned_glyph.x),
                euclid::Length::new(positioned_glyph.y) + y_offset,
            );

            let target_rect = euclid::Rect::<f32, PhysicalPx>::new(
                euclid::point2(glyph.glyph_origin_x, -glyph.y.truncate() as f32 - glyph.height.get() as f32)
                    + global_offset + glyph_offset,
                glyph.size().cast(),
            );
            if !target_rect.intersects(&clip) { continue; }
            let Some(target_rect) = target_rect.try_cast::<i16>() else { continue };
