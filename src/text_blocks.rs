...
pub struct Typography {
    ...
    pub ruby_scale: f32,
    /// ルビの帯が入るだけの行送りを確保するか（要件 7.8、2026-09-16、書き手の決定）。
    ///
    /// **入なら、行と行のあいだはルビの厚みぶん以上になる。**帯は行の箱の中に収まり、
    /// ブロック（＝タイル）の外へ出ないので、段落の先頭の行でも読みが切れない。本の組み方も
    /// これで、行間はルビが入る前提で決まっている。読み方の設定でルビを読まないなら要らない
    /// 空きなので、そのときは切る。**幾何に効く**ので、組み直しの合図（`hash_typography`）に入る。
    pub ruby_room: bool,
...
pub struct Marks {
    ...
    pub comment: bool,
    /// 字の脇に出る印——傍点（圏点）と傍線（要件 7.8）。
    ///
    /// **旗であって箱ではない。**本文の字はそのまま見せたまま、その脇に打つ
    /// ——字を隠す[`Ornament`]とは逆の仕事である。だから太字や斜体と同じ側にいて、
    /// 同じように入れ子になれる。印そのものはタイル描画側で、この範囲が当たった
    /// 矩形へ打つ。
    ///
...
///
/// **青空文庫の注記が名指す種類をそのまま持つ。**`《《…》》`と`［＃「…」に傍点］`は
/// 点（`Dot`）、`［＃「…」に丸傍点］`のように種類を書けばその印になる。
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum Beside {
    #[default]
    None,
    /// 傍点。既定の印で、青空文庫の「傍点」と`《《…》》`がこれ。
    Dot,
    /// ゴマ傍点。
    Sesame,
    /// 丸傍点（黒丸）。
...
pub enum Ornament {
    ...
    /// 空けたところ（横書きは記法の下、縦書きは記法の左）に絵を描く。
    Image {
        key: u64,
        width: u32,
        height: u32,
